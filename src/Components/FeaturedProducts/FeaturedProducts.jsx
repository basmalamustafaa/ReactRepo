...
export default function FeaturedProducts() {
    ...
    //USING REACT QUERY IS BETTER TO CACHE DATA
    //WHen using react query ,we don't need useEffect cause the function is get called when rendeing for first time and save the data in the cache

    // function getProducts() {
    //     return axios.get(`${BaseUrl}/api/v1/products`)
    // }
    // const { data, isLoading, isError, isFetched } = useQuery('products', getProducts, {
    //     cacheTime: 30000
    // }) //key inside queryClient is unique and used only once 
    //console.log(data?.data); //fel awl khals fel rendring byb2a lesa mfesh data fa lma b3ml data.data btb2a awl data btb2a undefined fa bytl3 error
    //fetching lw feh updates fel data byroh y3ml fetching
    ...
    let { isLoading, data } = GetApi('products', 'products')
    useEffect(() => {
        ...
        <div className='container '>
            ...
            {isLoading ?
                ...
                <>
                    ...
                    <form className="searchproducs mx-auto my-3"  >
                        <label className="searchproducs my-2 " htmlFor="search">
                            <input

                                className="search-input py-1 px-2 my-3"
                                type="search"
                                name="search"
                                id="search"
                                placeholder="Search..."
                    ...
                    <div className="row ">
                        {!filteredResults ?
                                        ...
                                        <div className='col-md-2 cart-box ' key={product._id}>
                                            ...
                                            <button className='bg-transparent border-0 py-3' style={{ width: "100%" ,display: "flex",justifyContent: "end"}} onClick={() => toggleWishlist(product._id)} >
                                                ...
                                                <Link className='ayhaga' to={`productdetails/${product._id}`}>
                                                    <div>
                                                        <div className='d-flex justify-content-end my-3'> {/* Added container */}
                            ...
                            <>
                                {filteredResults?.map((res) => (
                                    ...
                                    <div className="col-md-2" key={res._id}>
                                        <div key={res._id}>
                                            <div className='product p-3 cursor-pointer'>
                                            ...
                                            <button className='bg-transparent border-0 py-3' style={{ width: "100%" ,display: "flex",justifyContent: "end"}} onClick={() => toggleWishlist(res._id)} >
                                                ...
                                                <Link className='ayhaga' to={`productdetails/${res._id}`}>
                                                    <div>
                                                        <div className='d-flex justify-content-end my-3'> {/* Added container */}
                                                            {/* <i className='fa-solid fa-heart fa-xl' onClick={()=>AddtowishListFunc(res._id)}></i> */}
                                                        </div>
                        ...
                        {/* {
                isloading ? <div className='d-flex justify-content w-100 align-items-center  '>
                <ThreeDots 
                height="80" 
                width="80" 
                radius="9"
                ...
                <div className='col-md-2 ' key={product._id}>
                    ...
                    <button className='btn bg-main m-3 text-white'>
                        Add to cart
                    </button>
                    </div>
